...
export function setupSocketHandlers(io, gameState) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('move', (data) => {
            ...
            console.log('Bewegungsanfrage erhalten:', {
                playerId: socket.id,
                playerExists: !!player,
            ...
            if (!player || !player.canMove) {
                console.log('Bewegung abgelehnt:', {
                    reason: !player ? 'Spieler existiert nicht' : 'Spieler kann sich nicht bewegen'
                });
                return;
            }
            ...
            // Prüfe nur auf Kollisionen mit Wänden
            if (!checkCollision(newX - 10, newY - 10, 20, 20, walls)) {
                player.x = newX;
                player.y = newY;
                console.log('Spieler bewegt:', {
                    playerId: socket.id,
                    newPosition: { x: newX, y: newY }
                });
        
                // Sende die neue Position an alle Clients
                io.emit('playerMoved', { id: socket.id, x: newX, y: newY });
                io.emit('state', { players }); // Aktualisiere den Spielzustand
            } else {
                console.log('Bewegung blockiert durch Kollision');
            }
